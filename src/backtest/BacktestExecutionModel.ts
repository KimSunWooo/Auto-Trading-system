/**
 * Backtest-only local book execution model.
 * Not a product MockBroker and not used by live KIS PAPER trading runtime.
 */
...
export type BacktestBrokerMode =
  | "instant"
  | "delayed"
  | "reject"
  | "timeout"
...
/** Test env: BACKTEST_BROKER_MODE preferred (instant fills for historical replay). */
export function backtestBrokerMode(
  env: Record<string, string | undefined> = process.env,
): BacktestBrokerMode {
  const raw = String(env.BACKTEST_BROKER_MODE ?? "instant")
    .trim()
    .toLowerCase();
  if (
    raw === "delayed" ||
...
/**
 * Local book filler for historical strategy replay. driver reports "kis"
 * for IBroker typing only — never talks to KIS Open API.
 */
export class BacktestExecutionModel implements IBroker {
  ...
  forRule(ruleKey: string): BacktestExecutionModel {
    return new BacktestExecutionModel(this.box, ruleKey, this.source, this.sourceId, this.intent);
  }

  withSource(source: OrderSource, sourceId?: string): BacktestExecutionModel {
    return new BacktestExecutionModel(this.box, this.ruleKey, source, sourceId, this.intent);
  }

  withIntent(meta: IntentMeta): BacktestExecutionModel {
    return new BacktestExecutionModel(this.box, this.ruleKey, this.source, this.sourceId, meta);
  }
...
/** Prefer BacktestExecutionModel — kept temporarily for gradual test migration. */
export const BacktestBroker = BacktestExecutionModel;
