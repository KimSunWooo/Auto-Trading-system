...
/**
 * Parse a KIS realtime data line:
 *   0|H0STCNT0|{count}|{caret-separated fields}
 * or multi-row: fields may contain multiple records; we parse the first complete row.
 */
export function parseH0stCnt0Realtime(raw: string): ParsedH0stCnt0 | null {
  ...
  if (trId !== H0STCNT0_TR_ID) return null;

  const payload = segments.slice(3).join("|");
  const parts = payload.split("^");
  if (parts.length < H0STCNT0_COLUMNS.length) {
    // Some feeds may truncate trailing empty fields; require at least through BIDP1+volume.
...
/** Build official subscribe/unsubscribe frame. tr_type "1"=sub, "0"=unsub. */
