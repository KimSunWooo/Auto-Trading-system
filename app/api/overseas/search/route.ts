...
export async function GET(req: Request) {
  ...
  try {
    ...
    const found = parsed
      ? ((await client.search(parsed.symbol, parsed.exchange)) ?? parsed)
      : await client.search(
          q,
    ...
    const quote = await client.getQuote(found);
    return Response.json({
